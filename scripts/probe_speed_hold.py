...
def main():
    ...
    for label, cmd in COMMANDS:
        ...
        for _ in range(STEPS):
            ...
            # Canonical contract (matches experiment.rollout): record the
            # terminating step too (append-then-break), not drop it.
            ...
            if bool(state.done):
                break
        if not qpos:                       # fell on step 0 -> nothing to stack
            print(f"{label:12s}   (fell on step 0; skipped)")
            continue
        ...
        wb = metrics.power_breakdown(qfrc[s:][:, act], qvel[s:][:, act], dt, KT, R)
        elec = wb["elec_noregen"]
        ohm_share = 100 * wb["ohmic_share"]
        brake = wb["braking"]
        ...
        om_hip = qvel[s:][:, hip_dof]
        e_hip0 = metrics.power_breakdown(t0, om_hip, dt, KT, R)["elec_noregen"]
        e_hip1 = metrics.power_breakdown(t1, om_hip, dt, KT, R)["elec_noregen"]
        saved = e_hip0 - e_hip1
